...
exports.sendOtp = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      from: process.env.SMTP_USER,
      to: email,
      subject: 'Your HealthFirst OTP',
      html: `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          <h2 style="color: #4CAF50;">Your One-Time Password (OTP)</h2>
          <p>Hello,</p>
          <p>You requested to login to HealthFirst. Your OTP is:</p>
          <h1 style="color: #008CBA; background-color: #f2f2f2; padding: 15px; border-radius: 5px; text-align: center; font-size: 24px;">${otp}</h1>
          <p>This OTP is valid for ${otpExpirationMinutes} minutes.</p>
          <p>If you didn’t request this, please ignore this email.</p>
          <p>Thanks,<br/>HealthFirst Team</p>
        </div>
      `,
    };

    await sendEmail(mailOptions);
    res.status(200).json({ msg: 'OTP sent successfully.', otpExpirationMinutes });
  } catch (err) {
    console.error('Error in sendOtp:', err.message);
    res.status(500).json({ msg: 'Failed to send OTP. Check SMTP config.' });
  }
};
...
exports.verifyOtp = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      const defaultName = email.split('@')[0] || 'New User';
      user = new User({ email, name: defaultName, password: '' });
      try {
        await user.save();
        console.log('New user registered via OTP:', user.email);
      } catch (saveErr) {
        const validationErrors = Object.values(saveErr.errors || {}).map(e => e.message).join(', ');
        return res.status(500).json({ msg: `Error registering user: ${validationErrors || saveErr.message}` });
      }
    } else {
      console.log('Existing user logged in via OTP:', user.email);
    }
    ...
    jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '1h' }, async (err, token) => {
      if (err) return res.status(500).json({ msg: 'Failed to generate token.' });
